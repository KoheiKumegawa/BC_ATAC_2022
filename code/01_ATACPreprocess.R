...
#--------- analysis ---------#
#import fragments
bamList  <- read.csv("ref/bam_list.csv")
bamFiles <- bamList$bamfile %>% `names<-`(., bamList$pt)
...
QualityMetricsATAC <- lapply(names(fragments), FUN = function(x){
...
nucfree_fragments <- lapply(fragments, function(x){
  w <- width(x)
  out <- x[which(w < 100)]
})

#output
system("mkdir output/fragments_bed output/fragments_nucfree_bed/")
...
parallel::mclapply(names(nucfree_fragments), function(x) FragmentGRToBED(gr = nucfree_fragments[[x]], name = paste0("nucfree_", x), outputDir = "output/fragments_nucfree_bed/"), mc.cores = 10)
