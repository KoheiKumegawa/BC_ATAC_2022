#------------------------------------------------------------------------------
# 03 - NucFreeConstructATACseqCountMatrix.R
#------------------------------------------------------------------------------
...
#--------- analysis ---------#
system("mkdir output/inserts_nucfree_bed output/sample_peaks_nucfree_row/")

#read fragment bed as GRanges
FragFiles <- list.files("output/fragments_nucfree_bed/") %>% `names<-`(., gsub("nucfree_" ,"", gsub("-fragments.bed", "", .)))
fragments <- parallel::mclapply(FragFiles, function(x){
  fr <- fread(paste0("output/fragments_nucfree_bed/", x)) %>% data.frame(.)
...
parallel::mclapply(names(fragments), function(x){
  FragmentGRToInsert(gr = fragments[[x]], 
                     name = x, 
                     outputDirBED = "output/inserts_nucfree_bed/")
}, mc.cores = 10)

#Peak call for inserts
insertFiles <- list.files("output/inserts_nucfree_bed/") %>% `names<-`(., gsub("_inserts.bed", "", .))
parallel::mclapply(names(insertFiles), function(x){
  RunMacs2(inputBedPATH = paste0("output/inserts_nucfree_bed/", insertFiles[x]),
           inputBedName = x,
           genome = "hg38",
           outputDir = "output/sample_peaks_nucfree_row/",
           shift = -75,
           extsize = 150,
           method = "p",
           cutoff = 0.01)
...
gr_ls <- GenomicRanges::GRangesList(lapply(list.files("output/sample_peaks_nucfree_row/", pattern = "summits.bed", full.names = T), function(x) import.bed(x)))
names(gr_ls) <- gsub("_summits.bed", "", list.files("output/sample_peaks_nucfree_row/", pattern = "summits.bed"))
...
names(reproduciblePeaks) <- paste0("JFCRPeaks_nucfree_", c(1:length(reproduciblePeaks)))
mcols(reproduciblePeaks)$origName <- mcols(reproduciblePeaks)$name
mcols(reproduciblePeaks)$name <- names(reproduciblePeaks)

#add clinical info
ptinfo <- DataFrame(read.csv("ref/ptinfo.csv"))
ptinfo$ER <- as.factor(ptinfo$ER)
ptinfo$PgR <- as.factor(ptinfo$PgR)
ptinfo$HER2 <- as.factor(ptinfo$HER2)
ptinfo$Subtype <- as.factor(ptinfo$Subtype)
ptinfo$Primary_Relapse <- as.factor(ptinfo$Primary_Relapse)
ptinfo$HistologicalType <- as.factor(ptinfo$HistologicalType)
ptinfo$Ki67 <- ptinfo$Ki67*100
rownames(ptinfo) <- ptinfo$Patient
...
nucfree_se <- MakeATACSummarizedExperiment(fragmentGRangesList = fragments_named,
                                           unionPeaks = reproduciblePeaks,
                                           blacklist = blacklist,
                                           sampleName = rownames(ptinfo),
                                           by = "sample")
colData(nucfree_se) <- ptinfo
saveRDS(nucfree_se, "rds/nucfree_se.rds")